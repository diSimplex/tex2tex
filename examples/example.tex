...
\useurl[projG][https://www.gutenberg.org][][Project Gutenberg]
\useurl[eBook][https://www.gutenberg.org/files/1260/1260-h/1260-h.htm][][eBook]

\starttext

\subject{Pride and Prejudice\footnote{Written by Jane Austen, taken from
the \from[projG] \from[eBook] of the same name.}: Chapter 1}
...
Indeed you must go, for it will be impossible for us to visit him, if you
do not.”

“You are over scrupulous, surely. I dare say Mr. Bingley will be very
glad to see you; and I will send a few lines by you to assure him of my
hearty consent to his marrying whichever he chooses of the girls; though I
...
\startformula\startmathalignment[location=center]
\NC p(\theta^n, v^n | \vec{v}^{n-1}) \NC= p(v^n | \vec{v}^{n-1}) p(\theta^n | v^n, \vec{v}^{n-1}) \NR
\NC p(\theta^n, v^n | \vec{v}^{n-1}) \NC=  p(\theta^n | \vec{v}^{n-1}) p(v^n | \theta^n, \vec{v}^{n-1}) \NR
\NC p(\theta^n | \vec{v}^n ) \NC=  \frac{p(v^n | \theta^n, \vec{v}^{n-1}) p( \theta^n | \vec{v}^{n-1})}{p(v^n | \vec{v}^{n-1})} \NR
\NC \NC= \frac{p(v^n | \theta^n, \vec{v}^{n-1})}{p(v^n | \vec{v}^{n-1})} \times \sum_{j} p(\theta^n_i | \theta^{n-1}_j, \vec{v}^{n-1}) p(\theta^{n-1}_j | \vec{v}^{n-1}) \NR
\stopmathalignment\stopformula

\subject{JoyLoL parser}
